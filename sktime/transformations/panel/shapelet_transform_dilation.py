...
__all__ = ["ShapeletTransform", "RandomShapeletTransformDilation"]
...
class RandomShapeletTransformDilation(BaseTransformer):
    ...
    ...     RandomShapeletTransformDilation
    ... )
    >>> from sktime.datasets import load_unit_test
    >>> X_train, y_train = load_unit_test(split="train", return_X_y=True)
    >>> t = RandomShapeletTransformDilation(
    ...
    def __init__(
        ...
        super(RandomShapeletTransformDilation, self).__init__()
    
    @staticmethod
    def dilation(X: List, d: int) -> List:
        first = X[:, :, 0::d]
        for i in range(1, d):
            second = X[:, :, i::d]
    ...
    def _fit(self, X, y=None):
        ...
        Returns
        -------
        self : RandomShapeletTransformDilation
            This estimator.
        """
        ...
        shapelets = [[(-1.0, -1, -1, -1, -1, -1, -1)] for _ in range(self.n_classes)] # MOD dilation
        ...
        if time_limit > 0:
            while (
                ...
                for i, heap in enumerate(shapelets):
                    RandomShapeletTransformDilation._merge_shapelets(
                        heap,
                        candidate_shapelets,
                        max_shapelets_per_class,
                        i,
                ...
                if self.remove_self_similar:
                    for i, heap in enumerate(shapelets):
                        to_keep = (
                            RandomShapeletTransformDilation._remove_self_similar_shapelets(heap)
                        )
        ...
        self.shapelets = [
            (
                ...
                s[6], # an dieser Stelle wird die d_size an Stelle shapelets[7] gespeichert
            )
        ...
        to_keep = RandomShapeletTransformDilation._remove_identical_shapelets(self.shapelets)
    ...
    def _extract_random_shapelet(self, X, y, i, shapelets, max_shapelets_per_class):
        ...
        return quality, length, position, dim, inst_idx, cls_idx, d_size # MOD d_size wird zurückgegeben
        ...
        for i, series in enumerate(X):
            if i != inst_idx:
                distance = _online_shapelet_distance(
                    series[dim], shapelet, sorted_indicies, position, length, d_size
                )
            else:
                distance = 0
...
def _online_shapelet_distance(series, shapelet, sorted_indicies, position, length, d_size):
    ...
    while traverse[0] or traverse[1]:
        for n in range(2):
            mod = -1 if n == 0 else 1
            pos = position + mod * i
            traverse[n] = pos >= 0 if n == 0 else pos <= len(dilated_series) - length

            if not traverse[n]:
                continue

            start = dilated_series[pos - n]
            end = dilated_series[pos - n + length]
            ...
            for j in range(length):
                val = (dilated_series[pos + sorted_indicies[j]] - mean) / std if use_std else 0
