...
__all__ = ["RandomShapeletTransformDilation"]
...
class RandomShapeletTransformDilationOLD(BaseTransformer):
    ...
    def __init__(
        ...
        super(RandomShapeletTransformDilationOLD, self).__init__()
    ...
    def _fit(self, X, y=None):
        ...
        if self.max_shapelet_length is None:
            self._max_shapelet_length = self.series_length # TODO hier die max_shapelet_length reduzieren?  
        ...
        if time_limit > 0:
            while (
                ...
                candidate_shapelets = Parallel(n_jobs=self._n_jobs)(
                    ...
                    for i in range(self._batch_size) # dilation übergeben
                )

                for i, heap in enumerate(shapelets):
                    RandomShapeletTransformDilationOLD._merge_shapelets(
                        heap,
                        candidate_shapelets,
                        max_shapelets_per_class,
                        i,
                ...
                if self.remove_self_similar:
                    for i, heap in enumerate(shapelets):
                        to_keep = (
                            RandomShapeletTransformDilationOLD._remove_self_similar_shapelets(heap)
                        )
        ...
        self.shapelets.sort(reverse=True, key=lambda s: (s[0], s[1], s[2], s[3], s[4])) # hier evtl??

        to_keep = RandomShapeletTransformDilationOLD._remove_identical_shapelets(self.shapelets)
    ...
    def _extract_random_shapelet(self, X, y, i, shapelets, max_shapelets_per_class):
        ...
        dilation_x = random.uniform(0, np.log2(self.series_length/length))
        d_size = int(np.floor(pow(2, dilation_x)))

        X_dilated = self.dilation(X, d_size) # TODO dilation mapping
        ...
        quality = RandomShapeletTransformDilationOLD._find_shapelet_quality(
            X_dilated,
            y,
            shapelet,
            sorted_indicies,
